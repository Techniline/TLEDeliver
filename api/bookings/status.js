...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    const allowed = ['Pending','Assigned','In-Progress','Delivered','Cancelled'];
    if(!booking_id || !status || !allowed.includes(status)) return bad(res,'booking_id and valid status are required');

    const { error: uErr } = await s.from('bookings').update({ status }).eq('id', booking_id);
